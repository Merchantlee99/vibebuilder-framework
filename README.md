...
vibebuilder-os는 Codex와 ChatGPT를 더 일관되고 안전하게 쓰기 위한 프로급 바이브코딩 운영체계입니다.
...
vibebuilder-os는 이 문제를 줄이기 위해 만들었습니다. 목표는 속도를 죽이는 절차가 아니라, 재작업을 줄이고 결과물의 완성도를 올리는 구조를 만드는 것입니다.
...
flowchart TD
    ...
    B --> E["Readiness Check"]
    E --> F["Scope Freeze"]
    F --> G["Main Writer<br/>구현 담당 1명"]
    G --> H["Review Gate<br/>review"]
    H --> I["QA Gate<br/>qa 또는 browse"]
    I --> J["Security Gate<br/>cso가 필요할 때만"]
    J --> K["Ship Gate<br/>ship"]
    K --> L["Documentation.md 갱신"]

    M["AGENTS.md<br/>항상 켜진 헌법"] --- B
    M --- G
    N["SKILL.md<br/>반복 가능한 운영 절차"] --- B
    N --- H
    O["Subagent-Manifest.md<br/>이번 프로젝트 인력 배치표"] --- G
```
...
- `ChatGPT`: 아이디어 확장, 선택지 비교, 문제 재정의
- `Codex`: 저장소 맥락 기반 구현, 수정, 검증
...
- ChatGPT와 Codex를 같이 쓰지만 역할 분리가 필요한 사람
...
그래서 vibebuilder-os는 다음을 목표로 합니다.
...
이 저장소는 아래 구성 요소를 중심으로 채워집니다.

- `AGENTS.md`
- `skills/product-planner/SKILL.md`
- `skills/vibe-coding-workflow/SKILL.md`
- `templates/Prompt.md`
- `templates/Plan.md`
